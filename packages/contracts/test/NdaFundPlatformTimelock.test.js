...
describe("NdaFundPlatformTimelock Integration", function () {
  async function deployFixture() {
    const [owner, proposer, executor, user1, user2] = await ethers.getSigners();

    const MIN_DELAY = 2 * 24 * 60 * 60; // 2 days

    // Deploy NdaFundPlatformTimelock
    const NdaFundPlatformTimelock = await ethers.getContractFactory("NdaFundPlatformTimelock");
    const timelock = await NdaFundPlatformTimelock.deploy(
      MIN_DELAY,
    ...
    // Deploy NdaFundPlatformToken with owner as initial owner
