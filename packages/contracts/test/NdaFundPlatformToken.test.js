...
describe("NdaFundPlatformToken", function () {
    ...
    describe("Deployment", function () {
        it("Should set the correct name and symbol", async function () {
            ...
            expect(await token.name()).to.equal("NdaFundPlatform Token");
        ...
        it("Should revert if initialized with zero address", async function () {
            const NdaFundPlatformToken = await ethers.getContractFactory("NdaFundPlatformToken");
            await expect(
                upgrades.deployProxy(NdaFundPlatformToken, [ZERO_ADDRESS], {
                    initializer: "initialize",
                    kind: "uups",
                })
            ).to.be.revertedWithCustomError(NdaFundPlatformToken, "ZeroAddress");
        });
    });
    ...
    describe("Upgradeability", function () {
        it("Should be upgradeable by owner", async function () {
            ...
            const NdaFundPlatformTokenV2 = await ethers.getContractFactory("NdaFundPlatformToken");

            // Upgrade
            const upgraded = await upgrades.upgradeProxy(await token.getAddress(), NdaFundPlatformTokenV2);

            // Verify state is preserved
            expect(await upgraded.owner()).to.equal(owner.address);
            expect(await upgraded.name()).to.equal("NdaFundPlatform Token");
        });

        it("Should revert upgrade from non-owner", async function () {
            const { token, user1 } = await loadFixture(deployTokenFixture);

            const NdaFundPlatformTokenV2 = await ethers.getContractFactory("NdaFundPlatformToken", user1);

            await expect(
                upgrades.upgradeProxy(await token.getAddress(), NdaFundPlatformTokenV2)
