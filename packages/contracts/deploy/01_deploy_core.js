...
module.exports = async ({ getNamedAccounts, deployments }) => {
  ...
  await deploy("NdaFundPlatformBridge", {
      from: deployer,
      args: [config.lzEndpoint, swapAdapterAddress, config.usdc, factory.address, deployer],
      log: true
  });
  
  if ((await factoryContract.ndaFundPlatformBridge()) === ethers.constants.AddressZero) {
    log("Linking Bridge to Factory...");
    await (await factoryContract.updateBridge((await get("NdaFundPlatformBridge")).address)).wait();
  }
